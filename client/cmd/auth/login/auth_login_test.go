...
func TestLoginWithValidEmailAndPassword(t *testing.T) {
	...
	assert.Equal(t, "test_token", token, "Expected token_cache 'test_token', but got %s", token)
...
func TestLoginWithValidEmailAndPasswordButFailedToSaveToken(t *testing.T) {
	...
	mockTokenStorage := &MockTokenStorage{}
	mockTokenStorage.On("Save", token).Return(errors.New("failed to save token_cache"))

	err = mockTokenStorage.Save(token)
	assert.Error(t, err, "Expected error for failed to save token_cache")
...
func TestLoginWithValidEmailAndPasswordButTokenStorageReturnsError(t *testing.T) {
	...
	mockTokenStorage := &MockTokenStorage{}
	expectedError := errors.New("failed to save token_cache")
	mockTokenStorage.On("Save", token).Return(expectedError)

	err = mockTokenStorage.Save(token)
	assert.Error(t, err, "Expected error for failed to save token_cache")
	assert.Equal(t, expectedError, err, "Expected error 'failed to save token_cache', but got %v", err)
